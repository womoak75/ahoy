...
class RestApi {
...
#ifdef THIRDPARTY
            else if(path == "thirdparty")     onThirdpartyApiGet(request,root);
#endif
            else
...
#ifdef THIRDPARTY
                else if(path == "thirdparty")
                    root[F("success")] = onThirdpartyApiPost(obj,root);
#endif
                else {
    ...
    obj["name"][i++] = "Thirdparty";
#else
    obj["name"][i] = THIRDPARTY_MENUNAME;
#endif
    obj["link"][i++] = "/thirdparty";
#endif
        }

#ifdef THIRDPARTY
        void onThirdpartyApiGet(AsyncWebServerRequest* request, JsonObject root) {
            if(mApp->getThirdpartyApp()) {
                mApp->getThirdpartyApp()->onApiGet(request,root);
            } 
        }

        bool onThirdpartyApiPost(JsonObject request, JsonObject root) {
            if(mApp->getThirdpartyApp()) {
                return mApp->getThirdpartyApp()->onApiPost(request,root);
            } else {
                root[F("error")] = F("no thirdparty");
                return false;
            }
        }
#endif
