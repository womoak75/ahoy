...
#ifdef THIRDPARTY
            mWeb.on("/thirdparty",         HTTP_ANY,  std::bind(&Web::onThirdparty,this, std::placeholders::_1));
#endif
...
#ifdef THIRDPARTY
        void onThirdparty(AsyncWebServerRequest *request) {
            if(mApp->getThirdpartyApp()) {
                mApp->getThirdpartyApp()->onWebRequest(request);
            } else {
                AsyncWebServerResponse *response = request->beginResponse(200, F("text/html"), F("no thirdparty"));
                request->send(response);
            }
        }
#endif
