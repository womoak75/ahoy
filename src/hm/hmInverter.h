...
class inverterCb {
    public:
    virtual void onInverterValue(uint8_t inverterId, uint8_t fieldId, float value) = 0;
};
            ...
            if(NULL != rec) {
                ...
                if(NULL != rec) {
                    if(CMD_CALC != div) {
                        ...
                        if(nullptr != mInverterCb) {
                            mInverterCb->onInverterValue(id,rec->assign[pos].fieldId,rec->record[pos]);
                        }
                    }
                }
